...
import { ContactForm, type ContactFormValue } from '../../_components/ContactForm'

export const metadata = {
  title: 'Edit Contact · Dashboard',
}
...
export default async function EditContactPage({ params }: PageProps) {
  ...
  const initialValue: ContactFormValue = {
    label: toBilingual(contact.label),
    value: contact.value,
    hint: toBilingual(contact.hint),
  }
    ...
    <main className="relative min-h-screen w-full overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex max-w-4xl flex-col gap-8 px-4 py-8 sm:px-6 sm:py-10 lg:py-14">
        <header>
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Edit contact</h1>
          <p className="mt-1 text-sm text-white/60">{pickLocale(contact.label, 'en')}</p>
        </header>

        <ContactForm mode="edit" contactId={contact.id} initialValue={initialValue} />
      </div>
    </main>
  )
}
