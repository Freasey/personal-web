...
export const ContactView = ({ contacts, onBack }: ContactViewProps) => {
  return (
    <FadeSection className="w-full flex flex-col rounded-2xl overflow-hidden border border-white/10">
      ...
      {/* ── Header ── */}
      <div className="relative bg-white/5 px-8 pt-8 pb-10 overflow-hidden">
        {/* Watermark dekoratif */}
        <span className="pointer-events-none select-none absolute -right-3 -top-4 text-[11rem] font-black leading-none text-white/3">
          ✦
        </span>
        ...
        <div className="relative z-10 flex items-start justify-between">
          <div>
            ...
            <h2 className="text-[2.6rem] font-black leading-[1.05]">
              Let's Build
              <br />
          ...
          <button
            type="button"
            onClick={onBack}
            className="mt-1 rounded-full border border-white/15 px-4 py-2 text-xs text-white/45 transition-all duration-200 hover:border-white/30 hover:text-white cursor-pointer"
          >
            Back to menu
          </button>
        </div>
      ...
      {/* ── Intro band ── */}
      <div className="flex items-center gap-5 border-b border-white/7 bg-white/2 px-8 py-5">
        <div className="h-10 w-[3px] shrink-0 rounded-full bg-linear-to-b from-white/50 to-white/05" />
      ...
      <div className="divide-y divide-white/5">
        {contacts.map((item, index) => (
          <div
            key={item.id}
            className="group flex cursor-default items-center gap-5 bg-black/20 px-8 py-5 transition-colors duration-150 hover:bg-white/4"
          >
            {/* Nomor urut */}
